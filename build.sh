...
arch="${1:-arm}"
docker run --rm -v $GOPATH/src/:/go/src:rw -w /go/src/github.com/blademainer/sync-cloud -e "GO111MODULE=on" -it golang:1.11  bash -c "mkdir -p bin; GOOS=linux GOARCH=${arch} CGO_ENABLED=1 go build -o bin/${arch}/raft_example ./pkg/raft_example"
