...
# CGO_ENABLED=1
arch="${1:-arm}"
docker run --rm -e GOPROXY=https://goproxy.io -v $GOPATH/src/:/go/src:rw -w /go/src/github.com/blademainer/sync-cloud -e "GO111MODULE=on" -it golang:1.11  bash -c "rm -f go.sum; mkdir -p bin; GOOS=linux GOARCH=${arch}  go build -o bin/${arch}/raft_example ./pkg/raft_example"
